  ...
  <div class="qa-table-container">
    ...
    >
      <v-chip v-if="!tenantPartitionKey" closable=""
        >Tenant is not selected</v-chip
      >
      <v-chip v-else-if="!selectedKbId" closable=""
        >Knowledge base is not selected</v-chip
      >
      <v-chip v-else-if="!selectedTopicId" closable=""
        >Topic is not selected</v-chip
      >
    ...
    <div class="selection-info" v-if="tenantPartitionKey">
      ...
      <div class="data-controls">
        ...
        <v-btn
          ...
          @click="updateEditedItems"
          :disabled="getEditedCount() === 0"
          >Update {{ getEditedCount() }} Edited Item(s)</v-btn
        >
          ...
          <v-card-text class="pa-0">
            <div style="max-height: 240px; overflow-y: auto">
              <v-table density="compact">
                <thead>
                  <tr>
                    <th class="text-left font-weight-medium">Question</th>
                    <th class="text-left font-weight-medium">Answer</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, index) in ques" :key="index">
                    <td class="text-body-2">{{ item.que }}</td>
                    <td class="text-body-2">{{ item.ans }}</td>
                  </tr>
                </tbody>
              </v-table>
            </div>
