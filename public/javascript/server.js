...
var oConfigCall, txtPhoneNumber, txtCallStatus;
//txtRegStatus
var CallEarlyEpoch =
  (CallProfileEpoch =
  CallHangupEpoch =
...
function postInit() {
  ...
  oConfigCall = {
    ...
    sip_caps: [{ name: "+g.oma.sip-im" }, { name: "language", value: '"en"' }],
  };
}
...
function sipRegister(bullforcePstn) {
  ...
  */
  ...
  console.log("Registration Details : ", JSON.stringify(bullforcePstn));
  ...
  if (!SIPml.isInitialized) {
    console.log("SIPml not initialized yet");
  } else {
    ...
    case "i_new_call": {
      if (oSipSessionCall) {
        ...
        window.sipEventBridge.emit("incomingCall", {
          ...
          timestamp: new Date().toISOString(),
          sessionId: oSipSessionCall.getId
            ? oSipSessionCall.getId()
            : "unknown",
        });
...
function uiOnConnectionEvent(b_connected, b_connecting) {
  ...
  if (b_connected || b_connecting) {
    ...
    // sipPublish('open', 'Idle', 'I am Idle'); //NOT REQUIRED
  }
  if (!b_connected && !b_connecting) {
    // SelPresence.value='Offline';
    // sipPublish('closed', 'Offline', 'I am not Registered'); //NOT REQUIRED
  }
}
...
var onSipEventSession = function (e) {
  ...
  switch (e.type) {
    ...
    case "m_stream_audio_remote_added":
      console.log(
        "Call Audio added time : " +
          (CallEarlyMedia - window.performance.now()) / 1000 +
          " sec"
      );
      CallEarlyMedia = null;
    ...
    if (st == "PROFILE") {
      ...
      console.log(
        "Call Establishment time : " +
          (CallEngageEpoch - CallBeginEpoch) / 1000000 +
          " sec"
      );
    }
  }
...
function sipPublish(pRpid, pStatus, pNote) {
  if (oSipStack && pRpid && pStatus && pNote) {
    ...
    session.publish(content, contentType, {
      expires: 200,
      events_listener: { events: "*", listener: onPublishEvent },
      sip_caps: [
        { name: "+g.oma.sip-im" },
        { name: "+sip.ice" },
...
var onSubscriptionEvent = function (e) {
  ...
  if (e.type == "i_notify") {
    ...
    var xmlDoc = parser
      ? parser.parseFromString(e.getContentString(), "text/xml")
      : null;
    var presenceNode = xmlDoc
      ? xmlDoc.getElementsByTagName("presence")[0]
      : null;
    if (presenceNode) {
      ...
      if (dmNode) {
        ...
        if (noteNode) {
          subUserNote = noteNode.textContent;
        }
      }
    }
    console.log(
...
function sipSubscribe(sip_id) {
  ...
  subscriptionSession = oSipStack.newSession("subscribe", {
    expires: 200,
    events_listener: { events: "*", listener: onSubscriptionEvent },
    sip_headers: [
      { name: "Event", value: "presence" },
